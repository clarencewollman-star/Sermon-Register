...
- Reusable Texts, Vorraden, and People use `active` rather than deletion when they have history. Songs remain available and do not expose an Active/Inactive status.
...
6. **Song identity:** the Title, including any number, is the user-facing identity. If multiple hymnals later require otherwise identical titles, add a `hymnals` table and distinguish them in the selector.
